...
<script>
   ...
   console.log(sysInfo)
    ...
    let SCREEN_HEIGHT = sysInfo.windowHeight-100
	let PAGE_X, // 手按下的x位置
		PAGE_Y, // 手按下y的位置
			...
			loadImage: function () {
				...
				uni.getImageInfo({
					...
					success: function success(res) {
                        this.cropperInitW = res.width
                        this.cropperInitH = res.height
                        console.log(res)
						// IMG_RATIO = res.width / res.height
                        IMG_RATIO = 1
						if (IMG_RATIO >= 1) {
							IMG_REAL_W = SCREEN_WIDTH
							IMG_REAL_H = SCREEN_HEIGHT / IMG_RATIO
						} else {
						...
						if (IMG_RATIO >= 1) {
							...
							_this.setData({
								cropperW: SCREEN_WIDTH,
								cropperH: SCREEN_HEIGHT / IMG_RATIO,
								// 初始化left right
