<template>
	<view class="page">
		...
		<view class="top">
			<view class="img">
				<image src="../../static/img/topic/shangcheng.svg" mode="widthFix"></image>
			</view>
			<view class="top-text">
				<text>
					#星球特供商城，我們會聯合各大知名品牌聯合訂製商品，基於居民生活的日常用品，你可以告訴我們什麼商品是各位想要訂製的，居民獲得的PAN也可以在商城裡消費
		...
		<view class="user-section">
			<view class="user-item" v-for="(item,index) in topicList" :key="index">
				...
				<view class="content">
					<text class="content-text" decode="true"><text class="content-title" decode="true">#星球特供商城 &nbsp;&nbsp;</text>
						{{item.content}}</text>
				</view>
				<view class="option">
...
<script>
	...
	export default {
		...
		methods: {
			...
			publishDyn() {
				uni.navigateTo({
					url: '../publish/publish?tid=' + 2
				});
			},
			getTopicList() {
				checkTopicList(2, 0).then(data => {
					this.topicList = data
					console.log(data)
				})
		...
		onReachBottom() {
			checkTopicList(2,this.pageSize).then(data=>{
				this.pageSize+=10;
				if(!data){
