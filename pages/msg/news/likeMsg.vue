<template>
	<view class="like-page ">
		<text class="no-list" v-if="!likeList.length">暫無信息提示</text>
		<view class="list-item" v-for="(item,index) in likeList" :key="index">
			...
			<view class="list-item-right" @tap="toDetails(item.did,item.logImg)">
				<view class="item-userInfo" >
					...
					<text class="content">给你点{{item.likeCount}}次赞</text>
...
<script>
	...
	export default {
		computed: {
			likeList() {
				// console.log(this.$store.state.pushMsg.pushLikeMsg)
				return this.$store.state.pushMsg.pushLikeMsg
			}
		...
		methods: {
			...
			changeStatus(type) {
				upStatus(type).then(data => {
					// console.log(data)
					this.$store.state.pushMsg.pushLikeMsg[0].id = 0
