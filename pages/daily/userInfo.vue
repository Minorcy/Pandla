<template>
	<view class="user-page">
		<view class="userInfo">
			...
			<image class="portrait" :lazy-load="true" :src="userInfo.portrait" @tap="preview('por')" mode="aspectFill" v-else />
			<text class="user-name">{{userInfo.name}}</text>
			<text>{{userInfo.age}}/{{userInfo.stature}}cm/{{userInfo.weight}}kg</text>
		...
		<!-- <text v-if="dynInfo.length > 8 " class="more">点击查看更多</text> -->
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			this.uid = option.uid;
			if (option.distance) {
				this.distance = option.distance.substr(0, 3)
			}
			this.getUserInfo();
			this.getIsCon()
