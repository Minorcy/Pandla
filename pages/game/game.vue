<template>
	<view class="page">
		...
		<view class="top">
			...
			<view class="top-text">
				<text>
					#星球小遊戲，為了豐富居民日常娛樂，增強星球居民相互間互動性，我們將開發以社交為基礎的小遊戲，特向星球居民徵集意見。
		...
		<view class="user-section">
			<view class="user-item" v-for="(item,index) in topicList" :key="index">
				...
				<view class="content">
					<text class="content-text" decode="true"><text class="content-title" decode="true">#星球小遊戲 &nbsp;&nbsp;</text>
						{{item.content}}</text>
				</view>
				<view class="option">
...
<script>
	...
	export default {
		...
		methods: {
			...
			publishDyn() {
				uni.navigateTo({
					url: '../publish/publish?tid=' + 3
				});
			},
			getTopicList() {
				checkTopicList(3, 0).then(data => {
					this.topicList = data
					console.log(data)
				})
		...
		onReachBottom() {
			checkTopicList(3,this.pageSize).then(data=>{
				this.pageSize+=10;
				if(!data){
